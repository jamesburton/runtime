...
1. Check out a new branch and make the minimal edit (one test or one `.csproj` per PR when possible).
2. Run a focused build to confirm the change compiles: `./build.sh libs -c Release` is too heavy for the agent -- instead verify the edit syntactically with `grep`/`cat` and rely on CI to validate.
3. Open a draft PR with title `[mobile] <short description>`. The PR body must include: the build link, the failing test name, the Helix job+work item, the console log excerpt (sanitized), and the rationale for the fix class.
4. Post a comment on any related existing issue linking the PR.
